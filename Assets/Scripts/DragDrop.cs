...
public class DragDrop : MonoBehaviour
{
    ...
    [SerializeField] private bool _isOverDropZone = false;
    ...
    //
            ...
            switch (_gameManager.PlayersTurn)
            {
                ...
                case (false): // opponents turn
                    ...
                    if (collision.gameObject.name ==
                        "OpponentDropZone") // if they are over the opponent's drop zone, drop the card in
                    {
                        _dropZone = collision.gameObject;
                    }

                    if (collision.gameObject.tag == "Dropping Zone" &&
                        collision.gameObject.GetComponent<DroppingZone>().isEnemyZone ==
                        true) // if they are over the opponent's drop zone, drop the card in
                    {
                ...
                case (true): // local player's turn
                    ...
                    if (collision.gameObject.name ==
                        "DropZone") // if they are over the players drop zone, drop the card in
                    {
                        _dropZone = collision.gameObject;
                    }

                    if (collision.gameObject.tag == "Dropping Zone" &&
                        collision.gameObject.GetComponent<DroppingZone>().isPlayerZone ==
                        true) // if they are over the players drop zone, drop the card in
                    {
                        if (collision.gameObject.GetComponent<DroppingZone>().IsBeingUsed == false)
                        {
        ...
        private void OnCollisionExit2D(Collision2D collision)
        {
            ...
            switch (_gameManager.PlayersTurn)
            {
                ...
                case (false): // opponents turn
                    ...
                    if (collision.gameObject.tag == "Dropping Zone" &&
                        !_opponent.GetComponent<PlayerStats>().FieldCards
                            .Contains(
                                gameObject)) // if theyre no longer in the drop zone snap them back to opponents hand
                    {
                ...
                case (true): // local player's turn
                    if (collision.gameObject.tag == "Dropping Zone" &&
                        !_player.GetComponent<PlayerStats>().FieldCards
                            .Contains(
                                gameObject)) // if theyre no longer in the drop zone snap them back to players hand
                    {
                        _isOverDropZone = false;
                        _dropZone = null;
                        _droppingGridZone = null;
            ...
            if (_isOverDropZone && !_isDragging)
            {
                switch (_gameManager.PlayersTurn)
                {
                    case (true): // is the local player's turn
                        ...
                        if (_dropZone.name == "DropZone" &&
                            !_isDragging) // if the card is in the player's dropzone and not being dragged currently
                        {
                            if (!_attackCardSelected) // if there is no attack card selected 
                            {
                                if (belongsToPlayer == true &&
                                    !CardList.Contains(
                                        gameObject)) // card belongs to the player and this specific card is not already in the list 
                                {
                                    ...
                                    if (enemyCardinZone != null) // if there is an enemy card in the zone ahead
                                    {
                                        if (((belongsToPlayer && !_player.GetComponent<PlayerStats>().IsFirstTurn) ||
                                             (!belongsToPlayer &&
                                              !_opponent.GetComponent<PlayerStats>().IsFirstTurn)) &&
                                            !GetComponent<CardStats>().FirstTurnPlayed)
                                        {
                                            if (GetComponent<CardStats>().HasAttackedOpponent == false)
                                            {
                                                ...
                                                gameObject.GetComponent<CardStats>().HasAttackedOpponent =
                                                    true; // this card has attacked an opponent/opp card
                                            }
                                    ...
                                    else // if there is not an enemy card in the zone ahead
                                    {
                                        if (gameObject.GetComponent<CardStats>().HasAttackedOpponent == false &&
                                            !CardList[0].GetComponent<CardStats>().FirstTurnPlayed)
                                        {
        ...
        public void EndDrag()
        {
            ...
            if (_isOverDropZone && _gameManager.IsCardPlayable(gameObject,
